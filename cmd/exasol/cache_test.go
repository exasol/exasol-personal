...
func TestTextCommandReportsHowToReclaimSupersededCacheAfterIndexReplacement(t *testing.T) {
	...
	supersededPaths := resolver.Cache().SupersededPaths()
	if err := os.WriteFile(
	...
	// When
	addSupersededCacheCallToAction(supersededPaths)
	var stderr bytes.Buffer
...
//nolint:paralleltest // The terminal message queue is process-wide.
func TestTextCommandReportsHowToReclaimALegacyCacheDirectory(t *testing.T) {
	// Given
	legacyRoot := t.TempDir()
	if err := os.WriteFile(
		filepath.Join(legacyRoot, "index.json"), []byte(`{"version":1,"entries":{}}`), 0o600,
	); err != nil {
		t.Fatalf("write legacy index: %v", err)
	}
	cacheDir := t.TempDir()
	cache := resource.NewCache(
		cacheDir, filepath.Join(cacheDir, "resources.yaml"), legacyRoot,
	)
	resetTerminalMessages()
	defer resetTerminalMessages()

	// When
	addSupersededCacheCallToAction(cache.SupersededPaths())
	var stderr bytes.Buffer
	writeTerminalMessages(terminalConfig{stderr: &stderr, showCallsToAction: true})

	// Then
	output := stderr.String()
	if !strings.Contains(output, legacyRoot) ||
		!strings.Contains(output, "cache clean --all") {
		t.Fatalf("call to action = %q", output)
	}
}
