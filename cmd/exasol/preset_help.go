...
func embeddedLocalPresetNote() string {
	manifest, err := presets.ReadInfrastructureManifest("local")
	if err != nil {
		return ""
	}
	if strings.TrimSpace(manifest.Backend) != "local" {
		return ""
	}

	return "Special preset: `local` uses the built-in local backend instead of cloud " +
