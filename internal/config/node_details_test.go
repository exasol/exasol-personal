...
const testPresetKeyFileName = "preset-defined-key.pem"
...
func TestReadNodeDetails_PreservesPresetRenderedSSHCommand(t *testing.T) {
		...
		"nodes": {
			"n11": {
				...
				"ssh": {
					"username": "ubuntu",
					"keyName": "node-access",
					"keyFile": "preset-defined-key.pem",
					"port": "22",
					"command": "ssh -i preset-defined-key.pem ubuntu@db.example.local -p 22"
				},
				"tlsCert": "ignored",
				"database": {
					"dbPort": "8563",
	...
	require.Equal(t, testPresetKeyFileName, node.Ssh.KeyFile)
	require.Equal(
		t,
		"ssh -i preset-defined-key.pem ubuntu@db.example.local -p 22",
		node.Ssh.Command,
	)

	sshDetails, err := nodeDetails.GetSSHDetails("n11", deployment)
	require.NoError(t, err)
	require.Equal(
		t,
		filepath.Join(deployment.Root(), testPresetKeyFileName),
		sshDetails.KeyFile,
	)
}
...
func TestGetSSHDetails_AllowsAbsoluteLegacyKeyPath(t *testing.T) {
	...
	deploymentDir := t.TempDir()
	deployment := NewDeploymentDir(deploymentDir)
	absoluteKeyPath := filepath.Join(t.TempDir(), testPresetKeyFileName)
