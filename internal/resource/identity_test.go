...
func TestCache_CleanAllReclaimsLegacyCacheDirectory(t *testing.T) {
	t.Parallel()

	// Given
	legacyRoot := t.TempDir()
	cacheDir := t.TempDir()
	cache := NewCache(cacheDir, filepath.Join(cacheDir, cacheConfigFileName), legacyRoot)
	writeLegacyCacheFixture(t, legacyRoot, "opentofu")
	...
	if _, err := os.Stat(legacyRoot); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("expected the legacy cache directory to be removed, got %v", err)
	}
	if summary.RemovedBytes == 0 {
		t.Fatal("expected the reclaimed bytes to be reported")
	}
}

func TestCache_CleanAllKeepsADirectoryWithoutACacheIndex(t *testing.T) {
	t.Parallel()
	...
	cacheDir := t.TempDir()
	cache := NewCache(cacheDir, filepath.Join(cacheDir, cacheConfigFileName), legacyRoot)
	unrelated := filepath.Join(legacyRoot, "unrelated.txt")
	if err := os.WriteFile(unrelated, []byte("keep me"), filePerm); err != nil {
		t.Fatalf("write unrelated file: %v", err)
	}

	// When
	if _, err := cache.Clean(context.Background(), CleanOptions{Mode: CleanupModeAll}); err != nil {
		t.Fatalf("clean all: %v", err)
	}

	// Then
	if _, err := os.Stat(unrelated); err != nil {
		t.Fatalf("expected a directory without a cache index to be kept, got %v", err)
	}
}

func writeLegacyCacheFixture(t *testing.T, root, artifactName string) {
	t.Helper()

	if err := os.WriteFile(
		filepath.Join(root, cacheIndexFileName), []byte(`{"version":1,"entries":{}}`), filePerm,
	); err != nil {
		t.Fatalf("write legacy index: %v", err)
	}
	if err := os.WriteFile(
		filepath.Join(root, artifactName), []byte("legacy artifact"), filePerm,
	); err != nil {
