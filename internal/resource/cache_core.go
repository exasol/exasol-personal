...
// SupersededPaths reports cache directories an earlier launcher left behind: the
// current root when its index predates this version, and any legacy root that
// still holds an index. The index file is what distinguishes a cache directory
// from an unrelated directory sharing its name.
func (c *Cache) SupersededPaths() []string {
	var paths []string
	if c.SupersededIndexVersion() != 0 {
		paths = append(paths, c.root)
	}
	for _, legacyRoot := range c.legacyRoots {
		if holdsCacheIndex(legacyRoot) {
			paths = append(paths, legacyRoot)
		}
	}
...
func holdsCacheIndex(root string) bool {
	_, err := os.Stat(filepath.Join(root, cacheIndexFileName))
