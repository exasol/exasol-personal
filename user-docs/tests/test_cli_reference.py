...
def test_normalize_rewrites_windows_separators_in_home_paths() -> None:
    # Given a home directory and paths as a Windows host spells them
    home = PureWindowsPath(r"C:\Users\dev\AppData\Local\Temp\tmp1234")
    text = f"uses {home}\\.exasol\\personal\\deployments\\<name>\n"

    # When
    actual = cli_reference.normalize(text, home)

    # Then the page reads the same as it does on a POSIX host
    assert actual == "uses <home>/.exasol/personal/deployments/<name>"


def test_normalize_keeps_backslashes_outside_home_paths(tmp_path: Path) -> None:
    # Given
    text = f"uses {tmp_path}/config and prints \\n literally\n"

    # When
    actual = cli_reference.normalize(text, tmp_path)

    # Then
    assert actual == "uses <home>/config and prints \\n literally"
