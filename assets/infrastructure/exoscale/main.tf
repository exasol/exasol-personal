...
# We use a hash-based suffix to keep the bucket name unique and short.
locals {
  archive_bucket_tag_name = "${local.deployment_id}-sos-archive"
  # Use first 8 chars of MD5 hash for a short, deterministic suffix
  archive_bucket_suffix   = substr(md5(local.deployment_id), 0, 8)
  archive_bucket_id       = "${local.archive_bucket_tag_name}-${local.archive_bucket_suffix}"
