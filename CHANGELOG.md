...
- Presets can include a `resources.yaml` file. Its resources are available only
  ...
  name, and resolve relative local paths from the preset directory. A local
  resource is identified by the file's own state, so declaring a `sha256` for
  one is reported as a configuration error.
...
- Cached resources from an earlier version are no longer reused. The launcher starts a fresh cache, names the directories the previous contents occupy, and reclaims them with `exasol cache clean --all`. This covers the cache directory earlier versions wrote under its own name, which is recognized by the cache index file it holds.
