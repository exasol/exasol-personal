...
@pytest.mark.installation_e2e
@pytest.mark.local_e2e
def test_diag_cos_runs_confd_client(reusable_deployment: Deployment, infra: str) -> None:
    if infra == "local":
        pytest.skip("confd_client is a COS tool; local deployments use a VM shell fallback where it is not available")
...
def test_license_session_limit(reusable_deployment: Deployment, infra: str) -> None:
    if infra == "local":
        pytest.skip("Session limit enforcement is not yet implemented for local deployments")
