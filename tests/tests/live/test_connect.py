...
pytestmark = [
    ...
    pytest.mark.openspec("connect-result-retrieval"),
]
...
def test_connectable(shared_live_deployment: Deployment) -> None:
    # db_connectable() reads cloud node outputs, which local does not populate.
    assert shared_live_deployment.db_connectable()


def test_single_query(shared_live_deployment: Deployment) -> None:
    query: Final = "SELECT * FROM Dual"

    proc = shared_live_deployment.connect(input=query, capture_output=True)
...
def test_exit_command(shared_live_deployment: Deployment) -> None:
    queries: Final = [
        "exit",
        "SELECT * FROM Dual",
    ]
    ...
    proc = shared_live_deployment.connect(input=queries_str, capture_output=True)
    stdout = proc.stdout.strip()

    assert len(stdout) == 0


def test_multiple_queries(shared_live_deployment: Deployment) -> None:
    queries: Final = [
        "DROP SCHEMA IF EXISTS test_multiple_queries CASCADE;",
...
def test_file_import(shared_live_deployment: Deployment) -> None:
    people_csv_path: Final = Path(__file__).parent / Path("assets/people.csv")

    assert people_csv_path.exists()

    queries: Final = [
        "DROP SCHEMA IF EXISTS test_file_import CASCADE;",
...
@requires_posix_pty
def test_connect_table_width(shared_live_deployment: Deployment) -> None:
    queries: Final = [
        "DROP SCHEMA IF EXISTS test_connect_table_width CASCADE;",
...
def test_license_session_limit(shared_live_deployment: Deployment) -> None:
...
def test_password_marker_not_leaked_to_logs(shared_live_deployment: Deployment) -> None:
    ...
    deployment_log = Path(shared_live_deployment.deployment_dir.name) / "deployment.log"
...
def test_invalid_sql_does_not_crash_shell(shared_live_deployment: Deployment) -> None:
    ...
    proc = shared_live_deployment.connect(input=queries, capture_output=True)
...
def test_many_statements_remain_stable(shared_live_deployment: Deployment) -> None:
