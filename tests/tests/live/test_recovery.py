...
"""Fault-injection and recovery tests against managed live deployments."""
...
from tests.testcase_helpers import requires_macos_arm

pytestmark = pytest.mark.openspec("deployment-lifecycle-recovery")
...
def test_start_interrupt_sets_interrupted_state(
    ...
    stop_proc = reusable_live_deployment.stop_no_block()
    time.sleep(2)
    assert reusable_live_deployment.has_status(StatusOperationInProgress)
    ...
    assert reusable_live_deployment.has_status(StatusInterrupted)

    time.sleep(30)  # Allow tofu to release the lock

    # ========== GIVEN ==========
    # A stopped deployment
    assert reusable_live_deployment.stop().returncode == 0
    assert reusable_live_deployment.has_status(StatusStopped)

    # ========== WHEN ==========
    # We interrupt a start operation
    start_proc = reusable_live_deployment.start_no_block()
    time.sleep(2)
    assert reusable_live_deployment.has_status(StatusOperationInProgress)
    ...
    assert reusable_live_deployment.start().returncode == 0
...
def test_reconcile_vm_state_after_improper_shutdown(
    ...
    local_deployment.connect(input=CREATE_RECOVERY_DATA_SQL)
    ...
    status = json.loads(local_deployment.status().stdout)
    ...
    proc = local_deployment.connect(
        "--csv", input=QUERY_RECOVERY_DATA_SQL, capture_output=True
    )
...
@pytest.fixture
def interrupt_deployment(
    deployment_manager: DeploymentManager,
) -> Iterator[Deployment]:
    """Yield a fresh deployment that the test interrupts and then recovers."""
    deployment = deployment_manager.isolated_live(deploy=False)
    try:
        yield deployment
    finally:
...
def test_deploy_can_be_interrupted_and_recovered(
    ...
    # ========== THEN ==========
    if resume_succeeded:
        # On success the cluster reaches DatabaseReady and is reachable
