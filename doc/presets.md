...
- `node_access.pem`
  ...
  - Requirement: must be readable by the current user and typically needs mode `0600`. `deployment.json` should reference this key via `nodes[*].ssh.keyFile`, preferably as a path relative to the deployment directory.
