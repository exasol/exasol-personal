...
On macOS, the launcher manages a local VM and runs the same Podman-based database installation used on the host platforms inside it. If you do not configure VM memory explicitly, it defaults to about 50% of detected host memory; configured VM memory must be at least 4096 MB. On Linux and Windows, the database runs directly through host Podman and uses host-managed resources, so VM sizing options do not apply. Stopping or destroying a Windows deployment does not stop or remove the shared Podman machine. The initial local database credentials are `sys` / `exasol`. The `exasol shell host` and `exasol shell container` commands are available for macOS local deployments but are not supported on Linux or Windows.
...
- **Local deployments on Windows** — the database runs through host Podman on amd64; the launcher can prepare Podman and its shared default machine with explicit approval for host changes.
