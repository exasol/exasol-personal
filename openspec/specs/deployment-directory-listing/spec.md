...
## Purpose
Define how `exasol deployments list` enumerates known deployment directories, reports their initialization status and preset identity, and indicates which one is currently active, so users can discover and manage multiple named deployment directories.
## Requirements
