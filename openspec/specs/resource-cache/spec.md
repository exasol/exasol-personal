...
A statically defined `file://` or bare local-path artifact SHALL NOT be required to declare a checksum, since its integrity comes from being part of the same versioned repository commit as the launcher itself, not from a hand-authored value. Such an artifact is identified by the file's own state, so a checksum declared for one SHALL be reported as a configuration error. A container image artifact whose reference names its content by digest SHALL NOT be required to declare one either, since the digest already is that checksum.
...
#### Scenario: Local-path artifact declaring a checksum is rejected
- **WHEN** a statically defined resource declares a `file://` or bare local-path artifact with
  a checksum
...
The launcher SHALL continue to operate when the resource cache was written by a different launcher version. A cache written by an earlier version SHALL be replaced rather than reused, and the launcher SHALL tell the user how to reclaim the space its contents still occupy. This SHALL cover a cache directory an earlier version wrote under its own name, which the launcher SHALL recognize by the cache index it holds. A cache written by a later version SHALL be reported with the action that resolves it.
...
#### Scenario: Cache directory from an earlier launcher version is named as reclaimable
- **WHEN** a cache directory an earlier launcher version wrote under its own name
  holds a cache index
- **AND** the user runs a command producing text output
- **THEN** the launcher names that directory and the command that reclaims its
  space

#### Scenario: Full cleanup reclaims a cache directory from an earlier launcher version
- **WHEN** a cache directory an earlier launcher version wrote under its own name
  holds a cache index
- **AND** the user invokes cleanup for all cached resources
- **THEN** the launcher removes that directory
- **AND** the cleanup result reports the space it reclaimed

#### Scenario: Directory holding no cache index is kept
- **WHEN** a directory in the launcher's cache location holds no cache index
- **AND** the user invokes cleanup for all cached resources
