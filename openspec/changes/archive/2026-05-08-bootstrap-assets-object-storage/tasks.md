...
- [x] 2.2 Generate HTTPS access URLs for uploaded bootstrap objects and switch AWS cloud-init overlay files from inline `content` to `source.uri`.
...
- [x] 3.2 Generate HTTPS access URLs for uploaded bootstrap objects and switch Azure cloud-init overlay files from inline `content` to `source.uri`.
...
- [x] 4.3 Verify bootstrap storage is updated on asset changes and removed on destroy.
