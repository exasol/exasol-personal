...
#### Scenario: Restrict STACKIT bootstrap storage to deployment servers
- **WHEN** a deployment is created with the STACKIT preset
- **THEN** the bootstrap object store is not publicly readable from arbitrary internet clients
- **AND** the generated HTTPS source URIs remain usable from the deployed servers through an object storage policy scoped to the servers' public IP addresses
- **AND** the preset manages the STACKIT bootstrap bucket and bootstrap objects through a Terraform S3-compatible provider configured against STACKIT Object Storage
...
- **AND** the provider design artifacts document that Exoscale SOS does not provide an instance-scoped private-access mechanism analogous to the AWS, Azure, and STACKIT presets
