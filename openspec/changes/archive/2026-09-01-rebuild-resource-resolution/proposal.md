...
- The runtime resource cache stores each resource in a single flat directory
  ...
  names the directories the superseded contents occupy and reclaims them with
  `exasol cache clean --all`. A cache created by a later launcher version is
  reported together with the actions that resolve it.
...
- A cached resource is identified by whatever its source can state about it
  ...
  offers only a weak entity tag continues to be re-fetched on every use. A
  local artifact is identified by the file's own state, so declaring a checksum
  for one is reported as a configuration error.
