...
The rename from runtime artifacts to resources also renames the cache directory,
so an upgrade leaves the whole previous cache behind under its old name rather
than an old index under the current one. Both are the same situation to the
user, so both are named in that message and removed by `cache clean --all`. A
directory only counts as a previous cache when it holds the cache index file,
which keeps an unrelated directory that happens to share the name out of a
destructive command, the same marker-file test that distinguishes a legacy
