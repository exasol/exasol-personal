...
- [x] 4.4 Identify a local archive by path, size, and modification time, so a
  ...
  extraction, and report a checksum declared for a local artifact as a
  configuration error, since the file's own state identifies it. Test the
  unchanged-reuse case, the changed-re-extract case, and that a local artifact
  declaring a checksum is rejected.
...
- [x] 5.1 Flatten cache storage to `artifacts/<hash(identity)>/`, excluding
  ...
  prior entries are not reused. Recognize a cache directory an earlier launcher
  version wrote under its own name by the cache index it holds, name it in the
  reclaim message, and remove it during full cleanup. Test that two subpath
  selections of one source share a fetch, that extracted and unextracted views
  share a download, and that a resource with no identity does not collide with
  another. Test that JSON listing retains creation time, source URL, and
  identity. Test that the reclaim message names an earlier version's directory,
  that full cleanup removes it and reports the reclaimed space, and that a
  directory holding no cache index is kept.
- [x] 5.2 Stage every fetch and every extraction under `staging/<temp>/` and move
  ...
  complete. Reduce the git source to the clone this leaves it, since every fetch
  now targets its own empty directory. Test that an interrupted materialization
  leaves no artifact reported as cached and that a later request materializes it
  again.
...
- [x] 11.1 Update `README.md`, `doc/development.md`, and `doc/architecture.md`
  for the resolved specification, the `<group>/<member>` naming convention,
  preset resource layering, the renamed cleanup flag, and the renamed packages,
  and add `CHANGELOG.md` entries for the renamed cleanup flag and the cache
  rebuild.
