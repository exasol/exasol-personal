...
The key constraint is that the ExaNano runtime already exists only as Linux payloads. The missing capability is the host-side virtualization and orchestration layer on macOS. That host-side layer must live inside the `exasol` launcher rather than depending on a separate native wrapper.
...
- Add versioned Linux ExaNano payload download, verification, and caching from a product-owned HTTP location.
